...
function log(request, begin, status, headers) {
  ...
  log += " - ";
  log += request.remoteUser || "-";
  log += " [" + printDate(now) + "] ";
  log += '"';
  log += request.requestMethod;
  log += " ";
  log += request.pathInfo;
  log += request.queryString.length === 0 ? "" : "?"+request.queryString;
  log += " ";
  ...
  request.gateway.errors.write(log);
}
